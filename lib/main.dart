  ...
  static Future<Map<String, dynamic>> getLive() async {
    try {
      ...
      if (res.statusCode==200) {
        ...
        if (p>4000 && p<5000) { _last=p; return {'price':p,'bid':p-0.13,'ask':p+0.13,'status':'connected'}; }
      }
    } catch(e){}
    double change = (Random().nextDouble()-0.5)*0.8 - 0.05;
    _last += change;
    return {'price':_last,'bid':_last-0.13,'ask':_last+0.13,'status':'ticking'};
  }
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void fetch() async {
    ...
    String trend = m['price']> chartData[0]? 'Uptrend' : 'Downtrend';
    ...
    if(autoTradeEnabled.value && Random().nextDouble()>0.85) { TradeEngine.openAuto(m['price'], sig['signal'].toString()); }
  }
  @override
  Widget build(BuildContext context) {
    double price = market['price']?? PriceService.last;
    return Scaffold(appBar: AppBar(title: const Text('GoldMind AI')), body: ListView(padding: const EdgeInsets.all(12), children: [
      Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('XAUUSD - ${price.toStringAsFixed(2)} USD', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
        Text('Bid ${market['bid']?.toStringAsFixed(2)?? ''} / Ask ${market['ask']?.toStringAsFixed(2)?? ''} - ${market['status']?? ''}', style: const TextStyle(fontSize: 10, color: Colors.grey)),
        const SizedBox(height: 10),
        SizedBox(height: 60, child: CustomPaint(painter: MiniChartPainter(chartData), size: const Size(double.infinity, 60))),
        const SizedBox(height: 8),
        Text('Trend: ${signal['trend']?? ''} - RSI ${signal['rsi']?? ''} - Confidence ${signal['confidence']?? ''}%', style: const TextStyle(fontSize: 11)),
      ]))),
    ]));
  }
}
  ...
  static void updatePrices(double currentPrice) {
    for(var t in activeTrades){
      ...
      if((t['signal']=='BUY' && (currentPrice>=t['tp'] || currentPrice<=t['sl'])) || (t['signal']=='SELL' && (currentPrice<=t['tp'] || currentPrice>=t['sl']))){
        Future.delayed(const Duration(milliseconds: 500), ()=> closeTrade(t['id'] as int));
      }
    }
  }
}
...
class AnalysisPage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { return Scaffold(appBar: AppBar(title: const Text('Market Analysis')), body: const Center(child: Text('H4 H1 M15 - EMA 50/200 RSI ATR'))); }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Trade Signal')), body: ListView(padding: const EdgeInsets.all(16), children: [
      Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
        Text('BUY SETUP at ${price.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold)),
        ...
        ListTile(title: const Text('Take Profit'), trailing: Text((price+30).toStringAsFixed(2))),
        ValueListenableBuilder(valueListenable: autoTradeEnabled, builder: (_, auto, __)=> Column(children: [
          SwitchListTile(title: const Text('Auto Trading - Open/Close by itself'), value: auto, onChanged: (v){autoTradeEnabled.value=v; setState(() {});}),
          SizedBox(width: double.infinity, child: FilledButton(onPressed: (){ TradeEngine.openAuto(price, 'BUY'); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Trade Placed 0.01 lot'))); }, child: Text(auto? 'Auto Mode ON - Will trade randomly':'Place Paper Trade'))),
        ])),
      ]))),
    ]));
  }
...
class _ActiveTradesPageState extends State<ActiveTradesPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('Active Trades')), body: ListView(padding: const EdgeInsets.all(12), children: [
      ValueListenableBuilder(valueListenable: TradeEngine.balance, builder: (_, bal, __)=> Card(child: Padding(padding: const EdgeInsets.all(12), child: Text('Balance: ${bal.toStringAsFixed(2)} USD - Active: ${TradeEngine.activeTrades.length}')))),
      for(var t in TradeEngine.activeTrades) Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('XAUUSD ${t["signal"]} 0.01 lot'), Text('${t["pnl"].toStringAsFixed(2)} USD', style: TextStyle(color: (t["pnl"] as double)>=0?Colors.green:Colors.red, fontWeight: FontWeight.bold))]),
        Text('Entry ${(t["entry"] as double).toStringAsFixed(2)} - Now ${(t["current"] as double).toStringAsFixed(2)}', style: const TextStyle(fontSize: 11, color: Colors.grey)),
        Align(alignment: Alignment.centerRight, child: TextButton(onPressed: ()=> setState(()=> TradeEngine.closeTrade(t["id"] as int)), child: const Text('Close Trade'))),
      ]))),
    ]));
  }
}
...
class HistoryPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Trade History - Starts at Zero')),
      body: TradeEngine.historyTrades.isEmpty
         ? const Center(child: Text('No trades yet - 0 trades - Run auto trade to generate history'))
          : ListView.builder(
              itemCount: TradeEngine.historyTrades.length,
              itemBuilder: (_, i) {
                var t = TradeEngine.historyTrades[i];
                String sig = t["signal"].toString();
                double entry = t["entry"] as double;
                double pnl = t["pnl"] as double;
                String time = t["time"].toString();
                String closeTime = (t["closeTime"]?? '').toString();
                return ListTile(
                  title: Text('XAUUSD $sig at ${entry.toStringAsFixed(2)}'),
                  subtitle: Text('$time - $closeTime'),
                  trailing: Text('${pnl.toStringAsFixed(2)} USD', style: TextStyle(color: pnl>=0?Colors.green:Colors.red)),
                );
              },
            ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('Settings')), body: ListView(padding: const EdgeInsets.all(16), children: [
      Card(child: Column(children: [
        ListTile(title: const Text('Theme Dark / Light'), trailing: ValueListenableBuilder(valueListenable: isDark, builder: (_, dark, __)=> Switch(value: dark, onChanged: (v)=> isDark.value=v))),
        const ListTile(title: Text('Trading Mode'), subtitle: Text('Paper Trading - Auto execution')),
        const ListTile(title: Text('Risk Per Trade'), subtitle: Text('1 percent - 1.5 USD for 100 USD')),
      ])),
      Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('MT5 Connection - Insert Account Details Freely', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        TextField(controller: loginCtrl, decoration: const InputDecoration(labelText: 'MT5 Login', border: OutlineInputBorder())),
        const SizedBox(height: 8),
        ...
        const SizedBox(height: 8),
        TextField(controller: serverCtrl, decoration: const InputDecoration(labelText: 'Server', border: OutlineInputBorder())),
        const SizedBox(height: 8),
        TextField(controller: tokenCtrl, decoration: const InputDecoration(labelText: 'MetaApi Token (for exact MT5 price)', border: OutlineInputBorder())),
        const SizedBox(height: 12),
        SizedBox(width: double.infinity, child: FilledButton(onPressed: (){ mt5Account.value={'login':loginCtrl.text,'password':passCtrl.text,'server':serverCtrl.text,'token':tokenCtrl.text,'status': loginCtrl.text.isEmpty? 'Not Connected':'Connected'}; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Saved - ${mt5Account.value["status"]}'))); }, child: const Text('Connect MT5 - Save Details'))),
      ...
      Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
        const Text('Reset to Zero', style: TextStyle(fontWeight: FontWeight.bold)),
        FilledButton.tonal(onPressed: (){ TradeEngine.historyTrades.clear(); TradeEngine.activeTrades.clear(); TradeEngine.balance.value=100.0; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reset to Zero - Balance 100 USD'))); }, child: const Text('Reset All to Zero Trades')),
      ]))),
    ]));
  }
}
